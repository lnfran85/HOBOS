...
daily_means <- all_data %>%
  filter(str_detect(site, "SJD1")
         | str_detect(site, "SJD2")
         | str_detect(site, "SEI1")
         | str_detect(site, "SEI2")
         | str_detect(site, "QUD1")
         | str_detect(site, "QUD2")
         | str_detect(site, "LSA1")
         | str_detect(site, "LSA2")
         | str_detect(site, "FAR1")
         | str_detect(site, "FAR2")) %>%
  #droplevels() %>%
...
write.csv(monthly_means, paste(out_dir, "monthly_means_data_by_site.csv", sep="/"), 
          row.names=FALSE) # this file is still uncleaned for outliers! be carefull and clean it before analyze it
write.csv(monthly_range, paste(out_dir, "monthly_range_data_by_site.csv", sep="/"), 
          row.names=FALSE) # this file is still uncleaned for outliers! be carefull and clean it before analyze it
write.csv(maxmin_average, paste(out_dir, "monthly_maxmin_data_by_site.csv", sep="/"), 
          row.names=FALSE) # this file is still uncleaned for outliers! be carefull and clean it before analyze it


# exporting plots
png("monthly_means_by_site.png", width = 800, height = 800)
plot_grid(tempplot, lightplot, labels = c('A', 'B'), label_size = 12, nrow=2)
dev.off()

png("monthly_average_daily_range_by_site.png", width = 800, height = 800)
plot_grid(tempplot1, lightplot1, labels = c('A', 'B'), label_size = 12, nrow=2)
dev.off()

png("monthly_average_maxmin_by_site.png", width = 800, height = 800)
